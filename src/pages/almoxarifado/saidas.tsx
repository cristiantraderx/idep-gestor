...
import { PackageMinus, Plus, Search, Loader2, Edit3, Trash2, RefreshCw, Package, CalendarDays, FileText, User } from "lucide-react";
...
export function MovSaidasPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [movRes, itensRes, unidRes] = await Promise.all([
        supabase.from("movimentos_estoque").select("*, itens(nome, codigo), unidades(nome, sigla)").eq("tipo", "saida").order("created_at", { ascending: false }),
  ...
  const itemOptions = itens.filter((i) => i.quantidade_atual > 0).map((i) => ({ value: i.id, label: `${i.nome}${i.codigo ? ` (${i.codigo})` : ""} - Disp.: ${i.quantidade_atual}` }));
  ...
  const handleSave = async () => {
    ...
    if (itemSel && qtd > itemSel.quantidade_atual) { setFormError(`Quantidade insuficiente em estoque. Disponível: ${itemSel.quantidade_atual}`); return; }
    setSaving(true);
    try {
      const payload = { item_id: form.item_id, unidade_id: form.unidade_id, tipo: "saida" as const, quantidade: qtd, documento: form.documento || null, origem_destino: form.origem_destino || null, data_movimento: form.data_movimento, observacoes: form.observacoes || null };
      if (editing) {
        ...
        if (itemAtual) { await supabase.from("itens").update({ quantidade_atual: Math.max(0, itemAtual.quantidade_atual - qtd) }).eq("id", form.item_id); }
      }
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="page-header">
        <div className="flex items-center gap-4">
          <div className="rounded-xl p-3 bg-yellow-50 dark:bg-yellow-950/50 text-yellow-600"><PackageMinus className="h-6 w-6" /></div>
          <div><h2 className="page-title">Saídas do Estoque</h2><p className="page-subtitle mt-1">Registro de saídas e retiradas de materiais do almoxarifado</p></div>
        </div>
        <Button onClick={openCreate} className="gap-2"><Plus className="h-4 w-4" /> Nova Saída</Button>
      </div>
      <Card><CardContent className="p-4"><div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="relative flex-1 w-full"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input type="text" placeholder="Buscar por item, destino ou documento..." value={search} onChange={(e) => setSearch(e.target.value)} className="h-9 w-full rounded-md border border-input bg-background pl-9 pr-3 text-xs" /></div>
      ...
      <Card><CardContent className="p-0">
        ...
        : filtered.length === 0 ? <div className="flex flex-col items-center justify-center py-16 text-center"><PackageMinus className="h-10 w-10 text-muted-foreground/50 mb-3" /><p className="text-sm font-medium text-foreground">Nenhuma saída registrada</p></div>
        : <div className="divide-y divide-border">{filtered.map((item, index) => (<motion.div key={item.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.02 }} className="group flex flex-col sm:flex-row sm:items-center gap-3 px-4 sm:px-6 py-4 hover:bg-accent/30 transition-colors">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <Badge variant="destructive" className="text-[9px]">-{item.quantidade}</Badge>
            </div>
            <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1">
              {item.origem_destino && <span className="flex items-center gap-1 text-[11px] text-muted-foreground"><User className="h-3 w-3" />Destino: {item.origem_destino}</span>}
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Editar Saída" : "Nova Saída"} size="lg"><div className="space-y-4">
        ...
        <Input label="Destino / Requisitante" value={form.origem_destino} onChange={(e) => setForm({ ...form, origem_destino: e.target.value })} placeholder="Nome do requisitante" disabled={saving} />
        <Input label="Documento" value={form.documento} onChange={(e) => setForm({ ...form, documento: e.target.value })} placeholder="Ex: Requisição #123" disabled={saving} />
        ...
        <DialogFooter><Button variant="outline" onClick={() => setModalOpen(false)} disabled={saving}>Cancelar</Button><Button onClick={handleSave} disabled={saving} className="gap-2">{saving && <Loader2 className="h-4 w-4 animate-spin" />}{editing ? "Salvar" : "Registrar Saída"}</Button></DialogFooter>
      </div></Dialog>
