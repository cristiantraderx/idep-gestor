...
import type { Certidao, Aluno, Unidade } from "@/integrations/supabase/types";
...
const TIPO_OPTIONS = [
  { value: "nada_consta", label: "Certidão de Nada Consta" },
  { value: "tempo_servico", label: "Certidão de Tempo de Serviço" },
  { value: "frequencia", label: "Certidão de Frequência" },
  { value: "conclusao", label: "Certidão de Conclusão" },
...
export function CertidoesPage() {
  const [certidoes, setCertidoes] = useState<(Certidao & { aluno_nome?: string; unidade_nome?: string })[]>([]);
  ...
  const [formData, setFormData] = useState({
    aluno_id: "",
    tipo: "nada_consta" as Certidao["tipo"],
    data_emissao: new Date().toISOString().split("T")[0],
    requerente: "",
    texto: "",
    status: "gerada" as Certidao["status"],
    observacoes: "",
    unidade_id: "",
  });
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [certidoesRes, alunosRes, unidadesRes] = await Promise.all([
        supabase
          .from("certidoes")
  ...
  const handleSave = async () => {
    setFormError("");
    if (!formData.aluno_id || !formData.unidade_id || !formData.requerente.trim()) {
      setFormError("Aluno, unidade e requerente são obrigatórios.");
      return;
    }
  ...
  const filtered = certidoes.filter((c) => {
    const matchesSearch =
      ...
      c.requerente.toLowerCase().includes(search.toLowerCase()) ||
    ...
    >
      ...
      <div className="page-header">
        <div className="flex items-center gap-4">
          <div className="rounded-xl p-3 bg-green-50 dark:bg-green-950/50 text-green-600">
            <ScrollText className="h-6 w-6" />
          </div>
          <div>
            <h2 className="page-title">Certidões</h2>
            <p className="page-subtitle mt-1">
              Emita e gerencie certidões acadêmicas e documentos oficiais
            </p>
          </div>
        </div>
        ...
        <CardContent className="p-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="relative flex-1 w-full">
              ...
              <input
                type="text"
                placeholder="Buscar por aluno, número, requerente ou tipo..."
                value={search}
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm font-medium text-foreground">
                {search ? "Nenhuma certidão encontrada" : "Nenhuma certidão emitida"}
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                {search ? "Tente alterar os termos da busca" : "Clique em \"Nova Certidão\" para emitir"}
              </p>
            </div>
          ) : (
            <div className="divide-y divide-border">
              {filtered.map((certidao, index) => (
                <motion.div
                  key={certidao.id}
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-0.5 mt-1">
                      ...
                      <span className="flex items-center gap-1 text-[11px] text-muted-foreground">
                        Requerente: {certidao.requerente}
                      </span>
                      <span className="flex items-center gap-1 text-[11px] text-muted-foreground">
                        <CalendarDays className="h-3 w-3" />
                        {formatFullDate(certidao.data_emissao)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={statusVariant[certidao.status] || "default"}
                      className="text-[10px]"
                    >
                      {STATUS_OPTIONS.find((s) => s.value === certidao.status)?.label || certidao.status}
                    </Badge>
                    <button
                      onClick={() => setDeleteConfirm(certidao.id)}
      ...
      <Dialog
        ...
        title={editingCertidao ? "Editar Certidão" : "Nova Certidão"}
        description={editingCertidao ? `Editando certidão #${editingCertidao.numero || ""}` : "Preencha os dados para emitir uma certidão"}
        size="xl"
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Select
              ...
              value={formData.tipo}
              onChange={(e) => setFormData({ ...formData, tipo: e.target.value as Certidao["tipo"] })}
              disabled={saving}
            />
          </div>
          ...
          {editingCertidao && (
            <Select
              ...
              onChange={(e) => setFormData({ ...formData, status: e.target.value as Certidao["status"] })}
              disabled={saving}
            />
          )}

          <Textarea
            label="Texto da certidão"
            value={formData.texto}
            onChange={(e) => setFormData({ ...formData, texto: e.target.value })}
            placeholder="Texto oficial da certidão..."
            rows={5}
            disabled={saving}
          />
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={saving} className="gap-2">
              ...
              {editingCertidao ? "Salvar alterações" : "Emitir certidão"}
            </Button>
          </DialogFooter>
        </div>
      </Dialog>
      ...
      <Dialog
        ...
        description="Tem certeza que deseja excluir esta certidão? Esta ação não pode ser desfeita."
        size="sm"
      >
        <DialogFooter>
